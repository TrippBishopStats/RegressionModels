...
## How it works
Consider the matrix $\textbf{A}=`r ltxA`$. We have to use Gaussian elimiation to
get this matrix into an upper triangular form. The row operations to do this
are:
$$
\begin{aligned}
-2R_1 + R_2\to R_2\\
...
The first row operation can be accomplished with the elementary matrix $M_3$,
...
\end{aligned}
$$
Now, we let $\pmb{U}\pmb{x}=\pmb{y}$ and then the last equation above becomes
$$
\pmb{L}\pmb{y} = \pmb{b}
$$
Now, we can easily and quickly solve this equation using forward substitution 
because we have $\pmb{L}$ and $\pmb{b}$. Once we have solved for $\pmb{y}$, we
can then solve $\pmb{U}\pmb{x}=\pmb{y}$ for $\pmb{x}$ using backward 
substitution.
