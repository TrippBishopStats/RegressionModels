...
#| layout-ncol: 2
#| fig-cap:
#|    - "Scatter plot of $X_2$ vs $X_1$. The flat LOESS line indicates that the two variables are uncorrelated."
...
tibble(
  ...
  annotate("text", label=TeX("$r=0$"),x=3.5,y=-5,size=6,hjust=0) +
  labs(
    x=TeX("$X_1$"),
    y=TeX("$X_2$")
  )
tibble(
  ...
  ggplot(aes(x,y)) + geom_point(alpha=0.4) +
  geom_smooth(method="loess", se=FALSE) +
  annotate("text",label=TeX(paste("$","r=",r,"$")),x=3.5,y=-5,size=6,hjust=0) +
  labs(
    x=TeX("$X_2$"),
    y=TeX("$Y$")
  )
...
Both predictors are now in the model and notice how much the slope of $X_1$ 
changes.
```{r}
lm(Y ~ X1 + X2, data=df) |> summary() |> tidy() |> 
  mutate(
...
are negatively correlated. This means that when $X_2$ is omitted from the model
that contains both $X_1$ and $Y$, the slope parameter estimate for $X_1$ will be
negatively biased.
...
Both predictors are now in the model and observe how much the slope of $X_1$ 
changes.
```{r}
lm(Y ~ X1 + X2, data=df) |> summary() |> tidy() |> 
  mutate(
