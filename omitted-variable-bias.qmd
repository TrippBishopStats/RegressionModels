...
In all of these models, the predictors are standardised so that the regression
coefficients will match the correlation coefficients. This makes it a little
easier to see what's going on when $X_2$ is omitted from the model.
:::
...
Notice now that the coefficient estimate for X2 has decreased compared to .
